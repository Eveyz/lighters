class AddStudentsWidget extends React.Component {
  ...
  switchMode() {
    if(this.state.mode === "search") {
      $.getJSON('/students', (response) => { this.setState({ mode: "browse", apiResponse: response }) });
    } else {
      this.setState({ mode: "search", apiResponse: [] });
    }
  }
  ...
  render() {
    ...
    var studentList = this.state.apiResponse.length > 0 ? <StudentList students={this.state.apiResponse} addStudent={this.addStudent.bind(this)} /> : '';
...
class Student extends React.Component {
  ...
  render() {
    return (
      <a href="javascript:;" onClick={this.addStudent.bind(this)} className="collection-item">{this.props.student[1] + this.props.student[2]}</a>
    )
  }
}
